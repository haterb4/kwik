...
func (br *BenchmarkRunner) handleConnection(conn kwik.Session) {
...
func (br *BenchmarkRunner) handleStream(stream kwik.Stream) {
	defer stream.Close()
...
func (br *BenchmarkRunner) runClient(clientID int, serverAddr string, config BenchmarkConfig, latencies chan<- RequestLatency, tlsConfig *tls.Config) {
	...
	for i := 0; i < poolSize; i++ {
		conn, err := kwik.DialAddr(context.Background(), serverAddr, clientTLSConfig, &quic.Config{
			MaxIdleTimeout:  30 * time.Second,
			KeepAlivePeriod: 15 * time.Second,
		})
		if err != nil {
...
func (br *BenchmarkRunner) sendMessage(conn kwik.Session, payload []byte, networkLatency time.Duration) bool {
